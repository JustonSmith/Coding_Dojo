...
def edit(recipe_id):
    mysql= connectToMySQL("recipes")
    query= "UPDATE content SET title = %(title)s, description = %(desc)s WHERE id= %(recipe_id)s;"
    data = {
...
def add_new():
    mysql = connectToMySQL("exam_schema")
    query="INSERT INTO content (title, description, users_id) VALUES (%(title)s, %(desc)s, %(user)s);"
    data={
