...
<body>
    <div id="container">
        ...
        <c:if test="${instate.size() == 0}">
            <h5>There are currently no events in your area...</h5>
        </c:if>
        <c:if test="${instate.size() > 0}">
            <table class="table table-striped table-bordered table-hover">
                ...
                <tbody>
                    <c:forEach items="${instate}" var="in">
                        <tr>
                            <td><a href="/events/${in.id}">
                                    <c:out value="${in.name}" /></a></td>
                            <td>
                                <fmt:formatDate pattern="MMMM dd, yyyy" value="${in.date}" />
                            </td>
                            <td>
                                <c:out value="${in.location}" />
                            </td>
                            <td>
                                <c:out value="${in.user.firstName}" />
                            </td>
                            <c:choose>
                                <c:when test="${in.user == user}">
                                    <td>*Attending* | <a href="/events/${in.id}/edit">Edit</a> | <a
                                            href="events/${in.id}/delete">Delete</a></td>
                                </c:when>
                                <c:otherwise>
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                <c:forEach items="${outofstate}" var="out">
                    <tr>
                        <td><a href="/events/${out.id}">
                                <c:out value="${out.name}" /></a></td>
                        <td>
                            <fmt:formatDate pattern="MMMM dd, yyyy" value="${out.date}" />
                        </td>
                        <td>
                            <c:out value="${out.location}" />
                        </td>
                        <td>
                            <c:out value="${out.state}" />
                        </td>
                        <td>
                            <c:out value="${out.user.firstName}" />
                        </td>
                        <c:choose>
                            <c:when test="${out.user == user}">
                                <td>*Attending* | <a href="/events/${out.id}/edit">Edit</a> | <a
                                        href="events/${out.id}/delete">Delete</a></td>
                            </c:when>
                            <c:otherwise>
                                <c:set var="attending" value="${false}" />
